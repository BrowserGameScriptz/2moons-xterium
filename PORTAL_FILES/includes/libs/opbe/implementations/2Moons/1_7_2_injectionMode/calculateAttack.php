...
function calculateAttack(&$attackers, &$defenders, $FleetTF, $DefTF, $FleetTFD, $DefTFD, $isHostal = 0)
{
    ...
    foreach ($attackers as $fleetID => $attacker)
    {
        ...
        foreach ($attacker['unit'] as $element => $amount)
        {
			...
			$attackuser = $player['academy_p_b_1_1103'];
			...
			$shielduser = $player['academy_p_b_3_1303'];
			if($shielduser >= $shieldKritChance){
			$shieldKrit = 2; 
			} 
						
            $shipType = getShipType($element, $amount, $attackKrit, $shieldKrit);
    ...
    if ($report->defenderHasWin())
    {
        $won = DEFENDERS_WON;
		//attackers
		$debAtt = $report->getAttackerDebrisD();
		$debAttMet = $debAtt[0];
		$debAttCry = $debAtt[1];
		//defenders
            ...
            if ($playerGroup->existPlayer($info['player']['id']))
            {
                ...
                if ($player->existFleet($idFleet)) //if after battle still there are some ship types in this fleet
                {
                    ...
                    { 
                        ...
                        $shipType = $fleet->getShipType($idShipType,$amount,$attackKrit,$shieldKrit);
...
function getTechsFromArray($player)
{
    ...
    /* $attTech = $player['military_tech'] + $gouvernor_attack + $player['rpg_amiral'] + $player['combat_exp_level'] + $player['academy_p_b_1_1101'] + $player['hashallytech'];
    $shieldTech = $player['shield_tech'] + $gouvernor_shield + $player['rpg_amiral'] + $player['combat_exp_level'] + $player['academy_p_b_3_1301'] + $player['hashallytech'];
    $defenceTech = $player['defence_tech'] + $gouvernor_shield + $player['rpg_amiral'] + $player['combat_exp_level'] + $player['academy_p_b_3_1301'] + $player['hashallytech']; */
	
	$attTech = $player['military_tech'] + $gouvernor_attack + $player['rpg_amiral'] + $player['combat_exp_level'] + $player['academy_p_b_1_1101'] + (2*$player['academy_p_b_1_1102']) + $player['ally_fraction_armement'];
    $shieldTech = $player['shield_tech'] + $gouvernor_shield + $player['rpg_amiral'] + $player['combat_exp_level'] + $player['academy_p_b_3_1301'] + (2*$player['academy_p_b_3_1302']) + $player['ally_fraction_shields'];
    $defenceTech = $player['defence_tech'] + $gouvernor_shield + $player['rpg_amiral'] + $player['combat_exp_level'] + $player['academy_p_b_3_1301'] + (2*$player['academy_p_b_3_1302']) + $player['ally_fraction_armor']; 

...
class PlayerUtil
{
	...
	{
		$config	= Config::get($universe);
		$userId = database::get()->lastInsertId();
		$resourceQuery	= array();
		$params			= array(
			':username'				=> $userName,
			':email'				=> $userMail,
	...
	static public function maxPlanetCount($USER)
	{
		...
		return (int) ceil($config->min_player_planets + $USER['academy_p_b_2_1206'] + min($planetPerTech, $USER[$resource[124]] * $config->planets_per_tech) + min($planetPerBonus, $USER['factor']['Planets']));
	}

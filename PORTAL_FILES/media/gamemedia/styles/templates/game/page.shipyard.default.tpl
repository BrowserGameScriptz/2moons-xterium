...
<input name="methodPurchase" value="buy_resource" id="methodPurchase" type="hidden">
<div id="build_content" class="conteiner ship_build">
    <div id="fildes_band">
    	<a href="#" id="arrow_question" style="left:5px; right:auto;" onclick="return Dialog.manualinfo(3)" class="interrogation manual">?</a>
	   	<a href="game.php?page=Resourcecalc&options=200" title="Calculator" class="palanetarium_linck calculette" style="left:30px;top: 5px;width: 17px;height: 16px;"></a>
...
(adsbygoogle = window.adsbygoogle || []).push({});
    ...
    <div class="head">
		...
		<div id="darkp_{$ID}" class="prices" style="display:{if $buyId == $ID}block{else}none{/if}">
                            ...
                            <div class="price res921dm {if $Element.getRestPriceDM == 0}{else}required{/if}">
                    <div class="ico"></div>
                    <div class="text {if $Element.getRestPriceDM == 0}{else}tooltip{/if}" {if $Element.getRestPriceDM == 0}{else}data-tooltip-content="Не хватает: Металл 0"{/if}>{$Element.getElementPriceDM|number}</div>                                        
                </div>
        ...
        <div class="time_build" id="resp_time_{$ID}" style="display:{if $buyId == $ID}none{else}block{/if}">
				{if !$Element.techacc}{elseif $Element.elementTime == 0}{elseif $Element.UnitsSecond == 0}
					 ...
					 {/if}
					 {if $Element.techacc && $ID != 220}<div style="cursor: pointer;float: right;font-size: 8px;color: gray;" id="change_{$ID}" onclick="ChangeBuyMethod('{$ID}');">{if $buyId == $ID}{$LNG.customm_25}{else}{$LNG.build_dm}{/if}</div>{/if}
		...
		<div class="time_build" id="dm_time_{$ID}" style="display:{if $buyId == $ID}block{else}none{/if}">
				{if !$Element.techacc}{elseif $Element.elementTime == 0}{elseif $Element.UnitsSecond == 0}
					 ...
					 {$LNG.fgf_time}: <span class="time_build_text">Instant</span>
            	                    <a href="game.php?page=conveyors&amp;class=l" class="conveyors_up_item tooltip" data-tooltip-content="{$LNG.customm_15}">⇑</a>
					 {/if}
					 {if $Element.techacc && $ID != 220}<div style="cursor: pointer;float: right;font-size: 8px;color: gray;" id="change_{$ID}" onclick="ChangeBuyMethod('{$ID}');">{if $buyId == $ID}{$LNG.build_dm}{else}{$LNG.customm_25}{/if}</div>{/if}
		...
		{if $Element.AlreadyBuild}
			<div class="btn_build_border"><span class="btn_build red">{$bd_protection_shield_only_one}</span></div>
		{else}
			{if $NotBuilding && $Element.buyable}
			<div id="res_{$ID}" style="display: {if $buyId == $ID}none{else}block{/if};">
			...
			{/if}
			{if $NotBuilding && $Element.buyableDM}
			<div id="dm_{$ID}" style="display: {if $buyId == $ID}block{else}none{/if};">
				<div class="btn_build_border btn_build_border_left">
					<label title="{$LNG.bd_max_ships}" onclick="$('#input_dm_{$ID}').val('{$Element.maxBuildableDM}'); counting('{$ID}');" class="max_btn_ship">{$LNG.bd_max_ships}</label>
					<div class="div_text count_ships_dots">
						<input onkeyup="counting('{$ID}');" name="dark_dm[{$ID}]" id="input_dm_{$ID}" size="{$maxlength}" maxlength="{$maxlength}" value="0" class="text" tabindex="{$smarty.foreach.FleetList.iteration}" type="text">	
					</div>
				</div>
				<div class="btn_build_border btn_build_border_right">
					<input value="{$LNG.bd_build}" class="input_btn" type="submit">
    ...
    <div class="head">
        ...
        <div class="time_build" id="resp_time_{$ID}" style="display:{if $buyId == $ID}none{else}block{/if}">
				{if !$Element.techacc}{elseif $Element.elementTime == 0}{elseif $Element.UnitsSecond == 0}
					 ...
					 {$LNG.customm_13}: <span class="time_build_text tooltip" data-tooltip-content="<table class='tooltip_class_table'> <tbody><tr><th colspan='2'><font style='color:#a47d7a;'>{$LNG.tech.71}</font></th></tr> <tr> <td style='color: darkgray;'>Default</td><td >{$Element.valeurArray.Default|number}</td> </tr><tr> <td style='color: darkgray;'>{$LNG.lm_bonus} {$LNG.gl_galaxy} 7</td><td >{$Element.valeurArray.getGalaxySevenConv|number}</td> </tr><tr> <td style='color: darkgray;'>{$LNG.peace_1}</td><td >{$Element.valeurArray.peacefull_exp_light|number}</td> </tr><tr> <td style='color: darkgray;'>{$LNG.tech.71}</td><td >{$Element.valeurArray.SpecialShip|number}</td> </tr><tr> <td style='color: darkgray;'>{$LNG.all_devlopment}</td><td >{$Element.valeurArray.hashallyconv1|number}</td> </tr><tr> <td style='color: darkgray;'>{$LNG.premium_1}</td><td >{$Element.valeurArray.premium_conv_l|number}</td> </tr><tr> <td style='color: darkgray;'>{$LNG.planet_tele_pla}</td><td >{$Element.valeurArray.planetStructureBonuses|number}</td> </tr><tr> <td style='color: darkgray;'>{$LNG.lm_arsenal}</td><td >{$Element.valeurArray.arsenal_1_conv|number}</td> </tr><tr> <td style='color: darkgray;'>{$LNG.tech.604}</td><td >{$Element.valeurArray.rpg_technocrate|number}</td> </tr></tbody></table>">{$Element.UnitsSecond} {$LNG.customm_14}</span>
            	                    <a href="game.php?page=conveyors&amp;class=l" class="conveyors_up_item tooltip" data-tooltip-content="{$LNG.customm_15}">⇑</a>
					 {/if}
					{if $Element.techacc && $ID != 229}<div style="cursor: pointer;float: right;font-size: 8px;color: gray;" id="change_{$ID}" onclick="ChangeBuyMethod('{$ID}');">{if $buyId == $ID}{$LNG.customm_25}{else}{$LNG.build_dm}{/if}</div>{/if}
        </div>
                <div class="time_build" id="dm_time_{$ID}" style="display:{if $buyId == $ID}block{else}none{/if}">
						{if !$Element.techacc}{elseif $Element.elementTime == 0}{elseif $Element.UnitsSecond == 0}
			                     {$LNG.fgf_time}: <span class="time_build_text"> Instant </span> 
							 ...
							 {$LNG.fgf_time}: <span class="time_build_text">Instant</span>
		           	                    <a href="game.php?page=conveyors&amp;class=l" class="conveyors_up_item tooltip" data-tooltip-content="{$LNG.customm_15}">⇑</a>
							 {/if}
							 {if $Element.techacc && $ID != 229}<div style="cursor: pointer;float: right;font-size: 8px;color: gray;" id="change_{$ID}" onclick="ChangeBuyMethod('{$ID}');">{if $buyId == $ID}{$LNG.build_dm}{else}{$LNG.customm_25}{/if}</div>{/if}
		...
		{if !$Element.techacc}
		{elseif $ShowSpecialShips == 0 && $ID == 229 && $getGalaxySevenM7 == 0}<div class="btn_build_border"><span class="btn_build red">Premium Ship</span></div>
		{else}
		{if $Element.AlreadyBuild}
			<div class="btn_build_border"><span class="btn_build red">{$bd_protection_shield_only_one}</span></div>
		{else}
    ...
    <div class="head">
        ...
        <div class="time_build" id="resp_time_{$ID}" style="display:{if $buyId == $ID}none{else}block{/if}">
				{if !$Element.techacc}{elseif $Element.elementTime == 0}{elseif $Element.UnitsSecond == 0}
					 ...
					 {/if}
					{if $Element.techacc && $ID != 224}<div style="cursor: pointer;float: right;font-size: 8px;color: gray;" id="change_{$ID}" onclick="ChangeBuyMethod('{$ID}');">{if $buyId == $ID}{$LNG.customm_25}{else}{$LNG.build_dm}{/if}</div>{/if}
        </div>
		<div class="time_build" id="dm_time_{$ID}" style="display:{if $buyId == $ID}block{else}none{/if}">
						{if !$Element.techacc}{elseif $Element.elementTime == 0}{elseif $Element.UnitsSecond == 0}
			                     {$LNG.fgf_time}: <span class="time_build_text"> Instant </span> 
							 ...
							 {$LNG.fgf_time}: <span class="time_build_text">Instant</span>
		           	                    <a href="game.php?page=conveyors&amp;class=m" class="conveyors_up_item tooltip" data-tooltip-content="{$LNG.customm_15}">⇑</a>
							 {/if}
							 {if $Element.techacc && $ID != 224}<div style="cursor: pointer;float: right;font-size: 8px;color: gray;" id="change_{$ID}" onclick="ChangeBuyMethod('{$ID}');">{if $buyId == $ID}{$LNG.build_dm}{else}{$LNG.customm_25}{/if}</div>{/if}
		...
		{if !$Element.techacc}
		{elseif $ShowSpecialShips == 0 && $ID == 224 && $getGalaxySevenM19 == 0}<div class="btn_build_border"><span class="btn_build red">Premium Ship</span></div>
		{else}
		{if $Element.AlreadyBuild}
			<div class="btn_build_border"><span class="btn_build red">{$bd_protection_shield_only_one}</span></div>
		{else}
    ...
    <div class="head">
        ...
        <div class="time_build" id="resp_time_{$ID}" style="display:{if $buyId == $ID}none{else}block{/if}">
				{if !$Element.techacc}{elseif $Element.elementTime == 0}{elseif $Element.UnitsSecond == 0}
					 ...
					 {/if}
					{if $Element.techacc && $ID != 230}<div style="cursor: pointer;float: right;font-size: 8px;color: gray;" id="change_{$ID}" onclick="ChangeBuyMethod('{$ID}');">{if $buyId == $ID}{$LNG.customm_25}{else}{$LNG.build_dm}{/if}</div>{/if}
        </div>
		<div class="time_build" id="dm_time_{$ID}" style="display:{if $buyId == $ID}block{else}none{/if}">
						{if !$Element.techacc}{elseif $Element.elementTime == 0}{elseif $Element.UnitsSecond == 0}
			                     {$LNG.fgf_time}: <span class="time_build_text"> Instant </span> 
							 ...
							 {$LNG.fgf_time}: <span class="time_build_text">Instant</span>
		           	                    <a href="game.php?page=conveyors&amp;class=h" class="conveyors_up_item tooltip" data-tooltip-content="{$LNG.customm_15}">⇑</a>
							 {/if}
							 {if $Element.techacc && $ID != 230}<div style="cursor: pointer;float: right;font-size: 8px;color: gray;" id="change_{$ID}" onclick="ChangeBuyMethod('{$ID}');">{if $buyId == $ID}{$LNG.build_dm}{else}{$LNG.customm_25}{/if}</div>{/if}
		...
		{if !$Element.techacc}
		{elseif $ShowSpecialShips == 0 && $ID == 230 && $getGalaxySevenM32 == 0}<div class="btn_build_border"><span class="btn_build red">Premium Ship</span></div>
		{else}
		{if $Element.AlreadyBuild}
			<div class="btn_build_border"><span class="btn_build red">{$bd_protection_shield_only_one}</span></div>
		{else}
...
function ChangeBuyMethod(id) {
	for(id = 201; id < 230; id++) { 
		if(id == 220 || id == 229 || id == 224 || id == 230)
			continue;
		...
		var k2    = $('#dm_time_'+id);
		var TypeValue  = document.getElementById('methodPurchase').value;
		...
		else
		{
			...
			$('#change_'+id).html('{$LNG.customm_25}');
		}
	}
	
	if(TypeValue == "buy_darkmatter")
		$('#methodPurchase').val('buy_resource');
	else
		$('#methodPurchase').val('buy_darkmatter');
}
...
var CostInfo = {
{foreach $elementListall as $ID => $Element}
"{$ID}{literal}":["0","{/literal}{$LNG.tech.$ID}{literal}",{{/literal} {foreach $Element.costResources as $RessID => $RessAmount}{literal}"{/literal}{$RessID}{literal}":"{/literal}{$RessAmount}"{if !$RessAmount@last},{/if}{/foreach},"921" {literal}:{/literal} {$Element.getElementPriceDM} {literal}}]{/literal} {if !$Element@last},{/if}
{/foreach}
};

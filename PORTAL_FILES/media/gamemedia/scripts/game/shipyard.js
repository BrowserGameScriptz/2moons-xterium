...
function counting(id)
{
	...
	var Ellimet			= $(sitem);
	var it 				= 1;
	if(document.getElementById('methodPurchase').value != "buy_darkmatter")
		var	count 			= Math.max(0, Number(Ellimet.find('#input_'+(String(id))).val()));
	else if(document.getElementById('methodPurchase').value == "buy_darkmatter")
		var	count 			= Math.max(0, Number(Ellimet.find('#input_dm_'+(String(id))).val()));
	...
	if(count > MaxCount)
	{
		...
		if(document.getElementById('methodPurchase').value != "buy_darkmatter")
			...
			Ellimet.find('#input_dm_'+(String(id))).val(MaxCount);
	}
	...
	if(costRessources[921] > 0)
	{
		...
		if(document.getElementById('methodPurchase').value != "buy_darkmatter"){
			...
			Ellimet.find(".res921dm").find(".text").html(NumberGetHumanReadable(costRessources[921]));
			if(darkmatter < costRessources[921])	
			{
				Ellimet.find(".res921dm").addClass('required'); 
				Ellimet.find(".res921dm").find(".text").addClass('tooltip'); 
				Ellimet.find(".res921dm").find(".text").attr('data-tooltip-content', LNGning + ' ' + LNGtech921 + ' ' + NumberGetHumanReadable(costRessources[921] - darkmatter)); 
			}	
			else
			{
				Ellimet.find(".res921dm").removeClass('required')
				Ellimet.find(".res921dm").find(".text").removeClass('tooltip'); 
				Ellimet.find(".res921dm").find(".text").removeAttr('data-tooltip-content'); 
			}
		}
	...
	if(document.getElementById('methodPurchase').value != "buy_darkmatter")
		...
		Ellimet.find(".time_build_text").text("Instant.");

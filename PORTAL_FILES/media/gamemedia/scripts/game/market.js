...
function SEARCHAUCTION() /*ÐŸÐ¾Ð¸ÑÐº Ð°Ð¿Ð³Ñ€ÐµÐ¹Ð´Ð°*/
{
	$('#gr_search').attr("disabled", "disabled");
	setTimeout("$('#gr_search').removeAttr('disabled')", 3000);
	jQuery.post("./market.php", {'content_line': 'auction', 'max_cost': $('#max_cost').val(), 'min_count': $('#min_count').val(), 'name': $('#item_name').val(), 'ajax': 1}, 
	function(data)
	{
		var div = document.getElementById('market_content');
		if(data.CONTENT.HTML !== "undefined")
...
{
	var am 			= Number(String($('#current_antimatter').text()).replace(/[.]/g, ''));
	...
	{	
		...
		function(data)
		{
			if(data.MSG !== "undefined")
			{
				...
				if(!data.ERROR)
				{
					$('#current_antimatter').text(NumberGetHumanReadable(am - Cost));
				}
			}
		}, "json");
...
function BUYAUCTION(ID, Msg, NoMsg, Cost) /*ÐŸÐ¾ÐºÑƒÐ¿ÐºÐ° Ð´ÐµÐ¹Ñ‚ÐµÑ€Ð¸Ñ*/
{
	var am 			= Number(String($('#current_antimatter').text()).replace(/[.]/g, ''));
	...
	if(confirm(Msg))
	{	
		jQuery.post("./market.php", {'purchase_line': 'auction', 'id': ID, 'ajax': 1}, 
		function(data)
		{
			if(data.MSG !== "undefined")
			{

...
if (MODE === 'INGAME' || MODE === 'ADMIN')
{
	...
	if($config->game_disable == 0 || (isset($session->time_JSON) && $session->time_JSON > TIMESTAMP - 5)) 
		exit();
	
	$session->time_JSON = TIMESTAMP;
	
	$db		= Database::get();

	$sql	= "SELECT 
	user.id, user.authlevel, user.bana, sound_ataks,
	COUNT(message.message_id) as messages
	FROM %%USERS%% as user
	LEFT JOIN %%MESSAGES%% as message ON message.message_owner = user.id AND message.message_unread = :unread
	WHERE user.id = :userId
	...
	$USER	= $db->select($sql, array(
		':unread'	=> 1,
		':userId'	=> $session->userId
	));
	
	
	if(empty($USER) || $USER['bana'] == 1) 
		exit();
	}
	elseif (MODE === 'ADMIN')
	{

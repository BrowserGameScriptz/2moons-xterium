...
function ShowAccountDataPage()
{
	...
	$template->loadscript('assets/js/plugins/visualization/echarts/echarts.js');
	...
	$template->loadscript('assets/js/plugins/notifications/bootbox.min.js');	
	$template->loadscript('assets/js/plugins/notifications/sweet_alert.min.js');		
	$template->loadscript('assets/js/core/app.js');
	$template->loadscript('assets/js/charts/echarts/bars_tornados.js');
	$template->loadscript('assets/js/pages/user_pages_profile.js');
	...
	if (!empty($id_u))
	{
		...
		else
		{
			...
			$SpecifyItemsU	= 
			"u.id,u.username,u.email,u.email_2,u.authlevel,u.usertext,u.id_planet,u.galaxy,u.system,u.planet,u.user_lastip,u.ip_at_reg,u.darkmatter,u.antimatter,u.antimatter_bought,u.register_time,u.onlinetime,u.urlaubs_modus,u.
			...
			$alianzid		= $UserQuery['ally_id'];
			$usertext		= $UserQuery['usertext'];
			...
			$md	= "<a title=\"".pretty_number($UserQuery['antimatter'])."\">".shortly_number($UserQuery['antimatter'])."</a>";
			$mdd	= "<a title=\"".pretty_number($UserQuery['antimatter_bought'])."\">".shortly_number($UserQuery['antimatter_bought'])."</a>";
			...
			{
				$techoffi .= isset($techno[$i]) ? "<tr><td>".$LNG['tech'][$techno[$i]].": ".$UserQuery[$resource[$techno[$i]]]."</td>" : "<tr><td>&nbsp;</td>";
				
				$techoffi .= isset($officier[$i]) ? "<td>".$LNG['tech'][$officier[$i]].": ".$UserQuery[$resource[$officier[$i]]]."</td></tr>" : "<td>&nbsp;</td></tr>";				
			}
			...
			if ($alianza == 0 && $AliID == 0)
			{
				...
				$AllianceHave	= "<span class=\"no_moon\"> 
			...
			while ($PlanetsWhile	= $GLOBALS['DATABASE']->fetch_array($PlanetsQuery))
			{
				...
				{
					$Planettt = $PlanetsWhile['name']."&nbsp;(".$LNG['ac_moon'].")<br>["
								.$PlanetsWhile['galaxy'].":".$PlanetsWhile['system'].":".$PlanetsWhile['planet']."]";					
					
					$MoonZ	= 0;		
					$Moons = $PlanetsWhile['name']."&nbsp;(".$LNG['ac_moon'].")<br>["
								.$PlanetsWhile['galaxy'].":".$PlanetsWhile['system'].":".$PlanetsWhile['planet']."]";
					$MoonZ++;
				}
				else
				{
					$Planettt = $PlanetsWhile['name']."<br>[".$PlanetsWhile['galaxy'].":".$PlanetsWhile['system'].":"
								.$PlanetsWhile['planet']."]";
				}
				...
				{	
					...
					<tr>
						...
						<td>".pretty_number($PlanetsWhile['temp_min'])." / ".pretty_number($PlanetsWhile['temp_max'])."</td></tr>";
			...
			$visitList	= array();
			$QuerySearch	= $GLOBALS['DATABASE']->query("SELECT * FROM ".TRACKING." WHERE trackMode = 1 AND userId = ".$GLOBALS['DATABASE']->sql_escape($id_u)." ORDER BY time DESC;");
			while ($WhileResult	= $GLOBALS['DATABASE']->fetch_array($QuerySearch)){
				$visitList[$WhileResult['trackId']]	= array(
					'username'			=> $WhileResult['userName'],
					'pageVisited'		=> $WhileResult['pageVisited'],
					'time'				=> _date('F, d Y H:i:s', $WhileResult['time'] , $USER['timezone']),
				);	
			}
			
			$connexionList	= array();
			$ConnexionSearch	= $GLOBALS['DATABASE']->query("SELECT * FROM ".IPLOG." WHERE userId = ".$GLOBALS['DATABASE']->sql_escape($id_u)." ORDER BY timestamp DESC LIMIT 10;");
			while ($ConnexionResult	= $GLOBALS['DATABASE']->fetch_array($ConnexionSearch)){
				$connexionList[$ConnexionResult['suspectId']]	= array(
					'ipadress'			=> $ConnexionResult['ipadress'],
					'isValid'			=> $ConnexionResult['isValid'],
					'time'				=> _date('F, d Y H:i:s', $ConnexionResult['timestamp'] , $USER['timezone']),
				);	
			}
			...
			$Wrapper = array(
					...
					'antimatter'	=> $LNG['tech'][922],
					'antimatter_bought'	=> "Antimatter Bought",
					
					'authattack'	=> $LNG['qe_authattack'],
					'username'		=> $LNG['adm_username'],
					'field_max'		=> $LNG['qe_fields'],
			);
			$result    = $GLOBALS['DATABASE']->query("SELECT * FROM ".TRACKING." WHERE trackMode != 1 AND userId = ".$GLOBALS['DATABASE']->sql_escape($id_u)." ORDER BY TIME DESC");
			...
			{			
				...
				if(!empty($data)){
					...
					foreach ($data[0] as $key => $i) {
						$account_before[$key] = $i;
					}
					foreach ($data[1] as $key => $i) {
						$account_after[$key] = $i;
					}
					$LogArray1	=	array();
					foreach ($account_before as $key => $val) {
						if ($key != 'universe') {
							...
							$ElementTime	= array('urlaubs_until', 'bonus_timer', 'prem_res_days', 'prem_prod_from_colly_days', 'prem_prime_units_days', 'prem_o_build_days', 'prem_s_build_days');
							$LogArray1[]	= array(
								'Element'	=> $Element,
								'old'		=> (in_array($Element, $ElementTime) ? _date($LNG['php_tdformat'], $val) : (is_numeric($val) ? pretty_number($val) : $val)),
								'new'		=> (in_array($Element, $ElementTime) ? _date($LNG['php_tdformat'], $account_after[$key]) : (is_numeric($account_after[$key]) ? pretty_number($account_after[$key]) : $account_after[$key])),
							);
						}
					}
				}
				$LogArray[]	= array(
					'trackId'		=> $LogRow['trackId'],
					'LogArray1'		=> $LogArray1,
					'pageVisited'	=> $LogRow['pageVisited'],
					'admin'			=> $LogRow['userName'],
					'time'			=> _date($LNG['php_tdformat'], $LogRow['time'], $USER['timezone']),
				);
			}
			$GLOBALS['DATABASE']->free_result($result);
			
			$result    = $GLOBALS['DATABASE']->query("SELECT * FROM ".AMTRACKER." WHERE userId = ".$GLOBALS['DATABASE']->sql_escape($id_u)." ORDER BY trackTime DESC LIMIT 100");
			...
			{			
				$LogArray2[]	= array(
					'trackId'			=> $LogRow['trackId'],
					'tmpAntimatter'		=> $LogRow['tmpAntimatter'],
					'newAntimatter'		=> $LogRow['newAntimatter'],
					'trackDifference'	=> $LogRow['trackDifference'],
					'time'				=> _date($LNG['php_tdformat'], $LogRow['trackTime'], $USER['timezone']),
				);
			}
			$GLOBALS['DATABASE']->free_result($result);
	
			$template->assign_vars(array(
				'connexionList'					=> $connexionList,
				'LogArray'						=> $LogArray,
				'LogArray2'						=> $LogArray2,
				'log_log'						=> $LNG['log_log'],

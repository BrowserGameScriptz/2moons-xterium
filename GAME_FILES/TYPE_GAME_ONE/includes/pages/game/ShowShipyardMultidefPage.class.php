...
class ShowShipyardMultidefPage extends AbstractGamePage
{
	...
	function send(){
		...
		$allowedShipIDs	= array(401,402,403,404,405,406,407,408,409,410,411,412,413,414,415,416,417,418,419,420,421,422,502,503);
		//if($USER['id'] != 1)
			//$allowedShipIDs	= array(401,402,403,404,405,406,407,408,412,413,414,415,416,417,418,419,420,421,422);
		...
		if($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($shipID) && !empty($CountDefault) && in_array($shipID, $allowedShipIDs) && $CountDefault > 0)
		{
			...
			foreach($getPlayerPlanetsMoud as $Data){
				...
				if($Data['id'] != $PLANET['id']){
					...
					$ElementQueue 	= unserialize($Data['b_defense_id']);
				...
				if(!$requirementsMet){
					...
					continue;
				}elseif($isPremiumActive == 0 && in_array($shipID, array(420,421,422))){
				...
				}elseif($maxBuildQueue != 0 && $Count >= $maxBuildQueue)
				{
					...
					if($Data['id'] != $PLANET['id']){
						...
						if(empty($Count)){
							...
							//here it calc in php the real value with all bonus and widraw
	...
	function show()
	{
		...
		foreach($allowedShipIDs as $shipID)
		{
			if(in_array($shipID, $reslist['fleet']) || in_array($shipID, $reslist['defense']) || in_array($shipID, $reslist['missile'])) {
				...
				//js not correct code, its default so bad display
			}
		}

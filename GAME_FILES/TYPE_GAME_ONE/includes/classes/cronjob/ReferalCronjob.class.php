...
class ReferalCronjob implements CronjobTask
{
	...
	{		
		...
		if($config->referal_event < TIMESTAMP){
		$newevkaka = TIMESTAMP + 5*60;		
		$sql	= "UPDATE %%CONFIG%% SET referal_event = :referal_event WHERE uni = :uni;";
		$db->update($sql, array(
			':referal_event'	=> $newevkaka,
			':uni'	=> 1
		));
		...
		foreach($totalPremiums as $userInfo){
			...
			$message = '<span class="admin">'.$LNG['custom_referal'].'
			</span>';

...
class Session
{
	...
	public function selectActivePlanet()
	{
		...
		if(!empty($httpData))
		{
			...
			$planetId	= database::get()->selectSingle($sql, array(
				':userId'	=> $this->data['userId'],
				':planetId'	=> $httpData,
			), 'id');
			
			$sql	= 'SELECT ally_id, ally_rank_id FROM %%USERS%% WHERE id = :userId;';
			$allyFromPlayer	= database::get()->selectSingle($sql, array(
				':userId'			=> $this->data['userId'],
			));
			
			$sql = "SELECT * FROM %%ALLIANCE_RANK%% WHERE rankID = :allyId;";
			$allianceInfoSes = database::get()->selectSingle($sql, array(
				':allyId' => $allyFromPlayer['ally_rank_id'],
			));
			$sql = "SELECT * FROM %%ALLIANCE%% WHERE id = :allyId;";
			$allianceInfo2Ses = database::get()->selectSingle($sql, array(
				':allyId' => $allyFromPlayer['ally_id'],
			));
			
			$okAllowed = 1;
			if(empty($allianceInfoSes['PLANETS']) && $allianceInfo2Ses['ally_owner'] != $this->data['userId'])
				$okAllowed = 0;
			
			$sql	= 'SELECT id FROM %%PLANETS%% WHERE id = :planetId AND isAlliancePlanet = :isAlliancePlanet;';
			$planetId2	= database::get()->selectSingle($sql, array(
				':planetId'			=> $httpData,
				':isAlliancePlanet'	=> $allyFromPlayer['ally_id'],
			), 'id');
			...
			elseif(!empty($planetId2) && $okAllowed == 1)
			{
				$this->data['planetId']	= $planetId2;
			}
		}
	}

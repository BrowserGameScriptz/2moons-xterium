...
class PlayerUtil
{
	...
	static public function cryptNewPassword($action, $password) {
		$output = false;
	...
	static public function checkPosition($universe, $galaxy, $system, $position)
	{
		...
		return !(1 > $galaxy
			|| 1 > $system
			|| 1 > $position
			|| $config->max_galaxy - 1 < $galaxy
			|| $config->max_system < $system
			|| $config->max_planets < $position);
	}
	
	static public function createPlayer($universe, $userName, $userPassword, $userMail, $userLanguage, $validationID, $encodage = NULL, $deviceid = NULL, $galaxy = NULL, $system = NULL, $position = NULL, $name = NULL, $authlevel = 0, $userIpAddress = NULL)
	{
		$config	= Config::get($universe);
		...
		$params			= array(
			':id'					=> $validationID,
			':username'				=> $userName,
			':encodage'				=> !empty($encodage) ? $encodage : "",
			...
			':nameLastChanged'		=> 0,
			':darkmatter'			=> 50000,
			':ally_id'				=> config::get()->xteriumAllyId,
			':academy_p'			=> 1,
		...
		ally_id			= :ally_id,
		academy_p		= :academy_p,
		peacefull_last_update			= :peacefull_last_update;';
		...
		id_planet = :planetId,
		prem_res = :prem_res,
		prem_res_days = :prem_res_days,
		prem_o_build = :prem_o_build,
		prem_o_build_days = :prem_o_build_days,
		prem_button = :prem_button,
		prem_button_days = :prem_button_days
		WHERE id = :userId;";
		...
		$db->update($sql, array(
			...
			':planetId'	=> $planetId,
			':prem_res'	=> 500,
			':prem_res_days'	=> TIMESTAMP + 14 * 24 * 3600,
			':prem_o_build'	=> 2,
			':prem_o_build_days'	=> TIMESTAMP + 14 * 24 * 3600,
			':prem_button'	=> 3,
			':prem_button_days'	=> TIMESTAMP + 14 * 24 * 3600,
			':userId'	=> $userId,
		));
		...
		/* $sql = "INSERT INTO mantis_user_table SET
				username	= :username,
				email	= :email,
				password	= :password,
				enabled	= :enabled,
	...
	static public function createPlanet($galaxy, $system, $position, $universe, $userId, $name = NULL, $isHome = false, $authlevel = 0)
	{
		...
		foreach($AllowedRes as $elementID) {
	...
	static public function createMoon($universe, $galaxy, $system, $position, $userId, $chance, $diameter = NULL, $moonName = NULL)
	{
		...
		$diameter	= floor(pow(mt_rand(25, 35) + 3 * $chance, 0.5) * 1000); # New Calculation - 23.04.2011

		$maxTemperature = $parentPlanet['temp_max'];
		$minTemperature = $parentPlanet['temp_min'];
	...
	static public function deletePlayer($userId)
	{
		...
		$sql	= 'SELECT fleet_id FROM %%FLEETS%% WHERE fleet_target_owner = :userId;';
	...
	static public function maxPlanetCount($USER)
	{
		...
		return (int) ceil($config->min_player_planets + $USER['academy_p_b_2_1206'] + min($planetPerTech, $USER[$resource[124]] * $config->planets_per_tech));
	}
	...
	static public function sendMessage($userId, $senderId, $senderName, $messageType, $subject, $text, $time, $parentID = NULL, $unread = 1, $universe = NULL, $probeData = NULL, $circularPriority = 0)
	{
		if(is_null($universe))
		{
		...
		circularPriority	= :circularPriority;";
		...
		$db->insert($sql, array(
			...
			':unread'	=> $unread,
			':universe'	=> $universe,
			':probe_array'	=> $probeData,
			':circularPriority'	=> $circularPriority,
		));
	}
	
	static public function createAlliancePlanet($galaxy, $system, $position, $universe, $userId, $name = NULL, $authlevel = 0, $allianceId)
	{
		global $LNG;
		...
		$diameter			= (int) floor(1000 * sqrt(120));
		...
		temp_min 	= :minTemperature,
		temp_max 	= :maxTemperature,
		isAlliancePlanet 	= :isAlliancePlanet;';

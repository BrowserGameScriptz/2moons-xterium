...
class Logcheck
{
	...
	function save() {
		...
		$sql = "INSERT INTO %%TRACKING%% SET userId = :userId, userName = :userName, pageVisited = :pageVisited, data = :data, time = :time, trackMode = :trackMode;";
		database::get()->insert($sql, array(
			':userId'		=> $this->data['player'],
			':userName'		=> $this->data['username'],
			':pageVisited'	=> $this->data['pageLog'],
			':data'			=> $data,
			':time'			=> TIMESTAMP,
			':trackMode'	=> $this->data['mode'],
		));
	}
}
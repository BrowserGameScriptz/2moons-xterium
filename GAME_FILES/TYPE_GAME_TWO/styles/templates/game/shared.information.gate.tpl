...
<table style="width:100%;" class="tablesorter ally_ranks">
	<tbody>
		<tr style="height:20px;">
			<th colspan="3" class="left gray_stripe" >{$LNG.in_jump_gate_select_ships}</th>
		</tr>
		...
		{else}
			...
			{if !empty($gateData.gateList)}
				...
				<tr>
					<th class="gray_stripe">{$LNG.fl_ship_type}</td>
					<th class="gray_stripe">{$LNG.fl_ship_available}</td>
					<th class="gray_stripe"><a href="javascript:Gate.all();" style="color:#666;float:right;margin-right:5px">{$LNG.fl_select_all_ships}</a></td>
				</tr>
				...
				{/foreach}
				<tr>
					<td colspan="3" id="submit"><input value="{$LNG.in_jump_gate_jump}" type="button" onclick="Gate.submit();" style="width: 100%; margin-top: 5px;"></td>
				</tr>
			{else}
				<tr style="height:20px;">

...
<span onclick="$('#guaglione').slideToggle(0);" class="openCloseDetails"><img src="https://gf3.geo.gfsrv.net/cdnb6/577565fadab7780b0997a76d0dca9b.gif" height="16" width="16" class="flov7"></span>
    ...
    <div id="big_panet" style="background: url(//static.{$my_game_url}/media/gamemedia/styles/theme/gow/planeten/control_room_world2.png) no-repeat, url(//static.{$my_game_url}/media/gamemedia/styles/theme/gow/planeten/{$planetimage}.jpg) no-repeat ; background-size:cover;margin-top: 10px;">
...
<div name="situation_irreguliere" class="palnet_block_push palnet_luna_push blink_me1" {if $gal6modOver == 1}style="top:50px;"{/if}>
  ...
  <div style="width: 163.5px;max-width: 163.5px;float: left; height: 20px; overflow: hidden;">{if $buildBusy.tech}<img src="//static.{$my_game_url}/media/gamemedia/styles/images/iconav/tech.png"><span> {$LNG.tech[$buildBusy.tech['id']]} ({$buildBusy.tech['level']|number})</span>{/if}</div>
   <div style="width: 163.5px;max-width: 163.5px;margin-left:10px;float: left;height: 20px; overflow: hidden;">{if $buildBusy.buildings} <img src="//static.{$my_game_url}/media/gamemedia/styles/images/iconav/build.png"><span> {$LNG.tech[$buildBusy.buildings['id']]} ({$buildBusy.buildings['level']|number})</span>{/if}</div>
    <div style="width: 163.5px;max-width: 163.5px;margin-left:10px;float: left;height: 20px; overflow: hidden;">{if $buildBusy.fleet} <img src="//static.{$my_game_url}/media/gamemedia/styles/images/iconav/fleet.png"><span> {$LNG.tech[$buildBusy.fleet['id']]} ({$buildBusy.fleet['level']|number})</span>{/if}</div>
   <div style="width: 148px;height: 80%;margin-left:10px;float: left;height: 20px; overflow: hidden;">{if $buildBusy.defense} <img src="//static.{$my_game_url}/media/gamemedia/styles/images/iconav/shield.png"><span> {$LNG.tech[$buildBusy.defense['id']]} ({$buildBusy.defense['level']|number})</span>{/if}</div>
                    </div>
					{/if}
			...
			<a href="game.php?page=createMoon" style="color: #5ca6aa;text-decoration: blink;font-weight: bold;font-size: 13px;"><img src="//static.{$my_game_url}/media/gamemedia/styles/images/iconav/crealuna.png" style="height: 14px;width: 14px;"> {$LNG.over_creamoon}</a>
		</div>
		{elseif $planet_type == 1}
		<a href="game.php?page=overview&amp;cp={$Moon.id}&amp;re=0" class="palnet_block_info palnet_luna_info" title="">
            <img src="//static.{$my_game_url}/media/gamemedia/styles/theme/gow/planeten/moon.png" alt=" ({$LNG.planet_type.3})" height="60" width="60">
        </a>
		{else}
		<div class="palnet_block_info palnet_luna_info">
                <a href="game.php?page=kollaider" style="color: #5ca6aa;text-decoration: blink;font-weight: bold;font-size: 13px;"><img src="//static.{$my_game_url}/media/gamemedia/styles/images/iconav/miniera.png" style="height: 14px;width: 14px;"> {$LNG.over_collider}</a>
            </div>
		{/if}

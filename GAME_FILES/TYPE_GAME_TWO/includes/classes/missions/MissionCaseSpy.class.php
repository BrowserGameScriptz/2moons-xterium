...
class MissionCaseSpy extends MissionFunctions implements Mission
{
	...
	function TargetEvent()
	{
		...
		if($this->_fleet['fleet_start_planet'] == 21 && $this->_fleet['fleet_owner'] == 0 && $this->_fleet['fleet_mission'] == 6)
		...
		if($this->_fleet['fleet_target_owner'] != 0)
			$targetSpyTech	= max($targetUser['spy_tech'], 1);
		else
			$targetSpyTech	= max($senderUser['spy_tech'] + mt_rand(-5,5), 1);
		...
		$SpyDef			= $fleetAmount >= $MinAmount + 15 * SPY_VIEW_FACTOR;
		$SpyBuild		= $fleetAmount >= $MinAmount + 30 * SPY_VIEW_FACTOR;
		$SpyTechno		= $fleetAmount >= $MinAmount + 50 * SPY_VIEW_FACTOR;
			

		$classIDs[900]	= array(901,902,903,911);
		...
		if($SpyTechno && $this->_fleet['fleet_target_owner'] != 0) 
		{
		...
		foreach($classIDs as $classID => $elementIDs)
		{
			foreach($elementIDs as $elementID)
			{
				...
				else 
				{
					...
					if($elementID > 200 && $elementID < 400 && $targetPlanet[$resource[$elementID]] > 0)
						$probeData[]	= $elementID.','.$targetPlanet[$resource[$elementID]];
				}
			}
		...
		$template->assign_vars(array(
			...
			'isAlliancePlanet'	=> $targetPlanet['isAlliancePlanet'],
		...
		$probeData = !empty($probeData) ? implode(';', $probeData) : "0,0";
		PlayerUtil::sendMessage($this->_fleet['fleet_owner'], 0, $LNG['sys_mess_qg'], 0, sprintf($LNG['sys_mess_spy_report'], $targetPlanet['galaxy'], $targetPlanet['system'], $targetPlanet['planet']),
			$spyReport, $this->_fleet['fleet_start_time'], NULL, 1, $this->_fleet['fleet_universe'], $probeData);
		...
		if($this->_fleet['fleet_start_planet'] == 21){
			$this->KillFleet();
		}elseif ($targetChance >= $spyChance)
		{

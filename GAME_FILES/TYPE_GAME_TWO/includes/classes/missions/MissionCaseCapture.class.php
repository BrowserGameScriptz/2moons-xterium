...
class MissionCaseCapture extends MissionFunctions implements Mission
{
	...
	function EndStayEvent()
	{
		...
		if(empty($targetPlanet)){
			...
			$this->SaveFleet();
			return false;
		}elseif(empty($targetPlanet['id_owner']) && $targetPlanet['gal6mod'] == 1){
			$sql			= "UPDATE %%PLANETS%% SET id_owner = :id_owner WHERE id = :planetId;";
			database::get()->update($sql, array(
				':id_owner'	=> $this->_fleet['fleet_owner'],
			...
			$sql			= "UPDATE %%FLEETS%% SET fleet_target_owner = :fleet_target_owner WHERE fleet_end_id = :fleet_end_id;";
			database::get()->update($sql, array(
				':fleet_target_owner'	=> $this->_fleet['fleet_owner'],
				':fleet_end_id'			=> $this->_fleet['fleet_end_id']
			));
			
			$LNG			= $this->getLanguage($ownerUser['lang']);
			$message = "You succesfully captured the planet ".$targetPlanet['name'].". It will provide you the following bonuses.<br><br>".$LNG['galaxy6mod'][$targetPlanet['gal6type']];
			
			
			PlayerUtil::sendMessage($this->_fleet['fleet_owner'], 0, 'Empire Capture', 4, 'Empire Capture', $message, $this->_fleet['fleet_start_time'], NULL, 1, $this->_fleet['fleet_universe']);			
		}elseif(!empty($targetPlanet['id_owner']) && $targetPlanet['gal6mod'] == 1){
			$db				= Database::get();
...
HTML;
			...
			$FleetDebrisA 	= config::get()->Fleet_Cdr/100;
			$FleetDebrisD 	= config::get()->Fleet_Cdr/100;
			$DefDebrisA 	= 0;
			$DefDebrisD 	= 0;
			...
			if($ownerUser['ally_id'] != 0 && $targetUser['ally_id'] != 0 && $ownerUser['ally_id'] != $targetUser['ally_id'] && $targetUser['onlinetime'] > TIMESTAMP - 48 * 3600)
			...
			/* if($targetUser['onlinetime'] > TIMESTAMP - 7 * 24 * 3600){
				require_once 'includes/classes/missions/functions/calculateHonorPoints.php';
				$honorPoints = calculateHonorPoints($combatResult, $this->_fleet, $ownerUser, $targetUser, $fleetPointsDefenderHonor, $fleetPointsAttackerHonor, $totalDefenders);
			} */
			...
			/* if($targetPlanet['xterium_dock'] >= 1 && $totalWreckefender == 0){
			...
			foreach(array($userAttack, $userDefend) as $data)
			{
				...
				foreach($data as $userID => $userName)
				{
					...
					PlayerUtil::sendMessage($userID, 0, $LNG['sys_mess_tower'], 3, $LNG['sys_mess_attack_report'].' - '.sprintf($LNG['sys_adress_planet'],$this->_fleet['fleet_end_galaxy'],$this->_fleet['fleet_end_system'],$this->_fleet['fleet_end_planet']).' - ['.pretty_number($combatResult['unitLost']['attacker']-$combatResult['unitLost']['defender']).']', $message, TIMESTAMP, NULL, 1, $this->_fleet['fleet_universe']);

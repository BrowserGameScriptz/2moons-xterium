...
class MissionCaseRecycling extends MissionFunctions implements Mission
{
	...
	{	
		...
		$sql			= "SELECT der_metal, der_crystal FROM %%PLANETS%% WHERE id = :planetId;";
		$targetPlanet 	= database::get()->selectSingle($sql, array(
			':planetId'	=> $this->_fleet['fleet_end_id']
		));
		
		if($targetPlanet['der_metal'] + $targetPlanet['der_crystal'] == 0){
			$LNG		= $this->getLanguage(NULL, $this->_fleet['fleet_owner']);
		
			$Message 	= sprintf($LNG['sys_recy_gotten_3'], 
				GetTargetAddressLink($this->_fleet, ''),
				pretty_number(0), $LNG['tech'][901],
				pretty_number(0), $LNG['tech'][902]
			);

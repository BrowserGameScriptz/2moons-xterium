...
HTML;
		...
		$MaxPoints 		= ($topPoints < 5000000000) ? 10000000 : 50000000;
		...
		foreach($fleetArray as $shipId => $shipAmount){
			...
			if($angryEnnemie >= $angryEnnemieMax)
				$attackFactor	= mt_rand(100,110)/100;
			
			$targetFleetData[$shipId]	+= ceil($shipAmount * $attackFactor);
		}
		...
		if($combatResult['won'] == 'a'){
			foreach($arsenalItems as $Items){
				$itemChance = 3 + max(0,floor($ownerUser['combat_exp_level'] / 30));

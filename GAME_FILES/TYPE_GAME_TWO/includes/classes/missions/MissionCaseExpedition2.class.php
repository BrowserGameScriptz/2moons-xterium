	...
	function EndStayEvent()
	{
		...
		switch ($eventCase) { 
			...
			if(50 >= mt_rand(1,100)){
				...
				$FactorWeightPoint 	= 1 + 2500 / 1000;
				$FactorMaxRess		= 5 + 10 / 50;
				...
				if(89 <= $randomValue){
					$FactorRessMetal 		= mt_rand(10,50) * 5000 * $FactorMaxRess;
					$FactorRessCristal 		= mt_rand(5,25) * 5000 * $FactorMaxRess;
					$FactorRessDeutérium 	= (mt_rand(33,167)/10) * 5000 * $FactorMaxRess;
					/* $FactorRessMetal 		= mt_rand(50,250) * 5000 * $FactorMaxRess;
					$FactorRessCristal 		= mt_rand(25,125) * 5000 * $FactorMaxRess;
					$FactorRessDeutérium 	= (mt_rand(166,833)/10) * 5000 * $FactorMaxRess; */
					$MaxFleetPoints			= 5000000 * $FactorWeightPoint;
					$Message				= $LNG['sys_expe_found_ress_1_'.mt_rand(1,4)];
				}elseif(1 < $randomValue && 89 > $randomValue){
					$FactorRessMetal 		= mt_rand(52,100) * 5000 * $FactorMaxRess;
					$FactorRessCristal 		= mt_rand(26,50) * 5000 * $FactorMaxRess;
					$FactorRessDeutérium 	= (mt_rand(173,333)/10) * 5000 * $FactorMaxRess;
					/* $FactorRessMetal 		= mt_rand(260,500) * 5000 * $FactorMaxRess;
					$FactorRessCristal 		= mt_rand(130,250) * 5000 * $FactorMaxRess;
					$FactorRessDeutérium 	= (mt_rand(866,1666)/10) * 5000 * $FactorMaxRess; */
					$MaxFleetPoints			= 7500000 * $FactorWeightPoint;
					$Message				= $LNG['sys_expe_found_ress_2_'.mt_rand(1,3)];
				}elseif(0 <= $randomValue && 2 > $randomValue){
					$FactorRessMetal 		= mt_rand(102,200) * 5000 * $FactorMaxRess;
					$FactorRessCristal 		= mt_rand(51,100) * 5000 * $FactorMaxRess;
				    $FactorRessDeutérium 	= (mt_rand(280,460)/10) * 5000 * $FactorMaxRess;
					/*$FactorRessDeutérium 	= (mt_rand(340,667)/10) * 5000 * $FactorMaxRess;
					 $FactorRessMetal 		= mt_rand(750,1500) * 5000 * $FactorMaxRess;
					$FactorRessCristal 		= mt_rand(375,750) * 5000 * $FactorMaxRess;
					$FactorRessDeutérium 	= mt_rand(250,500) * 5000 * $FactorMaxRess; */
			...
			case 2:
				...
				if(10 < $eventSize) {
					...
					$Size		= mt_rand(320000, 750000);
					$Size			= $Size + ($Size / 100 * $premium_expe_bonus);
					$Message	= $LNG['sys_expe_found_ships_2_'.mt_rand(1,2)];
				} elseif(0 == $eventSize) {
					$Size	 	= mt_rand(550000, 1100000);
					$Size			= $Size + ($Size / 100 * $premium_expe_bonus);
...
HTML;
				...
				if($eventSize > 5){
					...
					$attackFactor			= mt_rand(50,57)/100;
					...
					$roundFunction	= 'ceil';
					$Message    			= $LNG['sys_expe_attack_1_3_2'];
					$attackFactor			= mt_rand(100,115)/100;
			...
			case 9:
				...
				$DefaultChance = 5 + (5 * $getBonCount['level']/2);				

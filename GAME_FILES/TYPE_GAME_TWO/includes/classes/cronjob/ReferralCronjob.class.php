...
class ReferralCronJob implements CronjobTask
{
	...
	{		
		...
		foreach($userArray as $user)
		{
			...
			$sql	= 'UPDATE %%USERS%% SET `darkmatter` = `darkmatter` + :bonus, `antimatter` = `antimatter` + :bonus1, `totalreferee` = `totalreferee` + :totalreferee WHERE `id` = :userId;';

			$db->update($sql, array(
				':bonus'	=> Config::get(ROOT_UNI)->ref_bonus,
				':bonus1'	=> Config::get(ROOT_UNI)->ref_bonus1,
				':totalreferee'	=> 1,
				':userId'	=> $user['ref_id']
			));
			...
			$Message	= sprintf($LNG['sys_refferal_text'], $user['username'], pretty_number($userConfig->ref_minpoints), pretty_number($userConfig->ref_bonus), $LNG['tech'][921], pretty_number($userConfig->ref_bonus1), $LNG['tech'][922]);
			...
			PlayerUtil::sendMessage(1, '', $LNG['sys_refferal_from'], 4, sprintf($LNG['sys_refferal_title'], $user['username']), $Message, TIMESTAMP);
		}

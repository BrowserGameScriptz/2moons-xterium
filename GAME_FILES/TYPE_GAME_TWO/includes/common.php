...
if (MODE === 'INGAME' || MODE === 'ADMIN' || MODE === 'CRON')
{
	...
	FROM %%USERS%% as user
	LEFT JOIN %%MESSAGES%% as message ON message.message_owner = user.id AND message.message_unread = :unread AND message.message_deleted = 0
	...
	if($USER['bana'] == 1 && $USER['banaday'] < TIMESTAMP) {
		$sql	= "UPDATE %%USERS%% SET bana = :bana, banaday = :banaday WHERE id = :userId;";
	...
	if (MODE === 'INGAME')
	{
		...
		if(empty($PLANET))
		{
			...
			if(empty($PLANET))
			{
				throw new Exception("Main Planet does not exist!".$USER['username']);
			}
			else
			{

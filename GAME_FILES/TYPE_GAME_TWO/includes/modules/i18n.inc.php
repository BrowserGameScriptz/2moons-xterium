...
FlySprayI18N::init('en', $LNG);
FlySprayI18N::setDefault($LNG);
...
function load_translations(){
	...
	$translation = 'lang/'.$lang_code.'.php';
	...
	}elseif( 'en'!=substr($lang_code, 0, strpos($lang_code, '_')) && is_readable('lang/'.(substr($lang_code, 0, strpos($lang_code, '_'))).'.php') ){
		# fallback 'de_AT' to 'de', but not for 'en_US'
		$translation='lang/'.(substr($lang_code, 0, strpos($lang_code, '_'))).'.php';
		include_once($translation);
		$language = is_array($translation) ? array_merge($language, $translation) : $language;    
	}

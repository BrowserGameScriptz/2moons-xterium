...
class BattleReport
{
    ...
    public function getAttackerDebrisA()
    {
        $metal = 0;
        $crystal = 0;
        foreach ($this->getAttackersLostUnits(!REPAIRED_DO_DEBRIS) as $idPlayer => $player)
        {
            foreach ($player as $idFleet => $fleet)
            {
                foreach($fleet as $role => $values)
                {
                    ...
                    {
                        $factor = constant(strtoupper($role).'_DEBRIS_FACTOR_A');				
                        $metal += $lost[0] * $factor;
                        $crystal += $lost[1] * $factor;
    ...
    public function getDefenderDebrisA()
    { 
        $metal = 0;
        $crystal = 0;
	...
	public function getAttackerDebrisD()
    {
        $metal = 0;
        $crystal = 0;
        foreach ($this->getAttackersLostUnits(!REPAIRED_DO_DEBRIS) as $idPlayer => $player)
        {
            foreach ($player as $idFleet => $fleet)
            {
                foreach($fleet as $role => $values)
                {
                    foreach ($values as $idShipType => $lost)
                    {
                        $factor = constant(strtoupper($role).'_DEBRIS_FACTOR_D');				
                        $metal += $lost[0] * $factor;
                        $crystal += $lost[1] * $factor;
    ...
    public function getDefenderDebrisD()
    {
        $metal = 0;
        $crystal = 0;
        foreach ($this->getDefendersLostUnits(!REPAIRED_DO_DEBRIS) as $idPlayer => $player)

...
function ShowBanPage() 
{
	...
	{
		$UserSelect['List']	.=	'<option '.(($a['bana']	==	'1') ? 'selected="selected"' : '').' value="'.$a['username'].'">'.$a['username'].'&nbsp;&nbsp;(ID:&nbsp;'.$a['id'].')</option>';
		$Users++;
	}
	...
	$template->loadscript('assets/js/plugins/forms/inputs/duallistbox.min.js');
	$template->loadscript('assets/js/core/app.js');
	$template->loadscript('assets/js/pages/form_dual_listboxes.js');

	$Name					= HTTP::_GP('ban_name', '', true);
	$BANUSER				= $GLOBALS['DATABASE']->getFirstRow("SELECT b.theme, b.longer, u.id, u.urlaubs_modus, u.isChat, u.banaday FROM ".USERS." as u LEFT JOIN ".BANNED." as b ON u.`username` = b.`who` WHERE u.`username` = '".$GLOBALS['DATABASE']->sql_escape($Name)."' AND u.`universe` = '".Universe::getEmulated()."';");
	...
	if(isset($_POST['panel']))
	{
		...
		$isChat	= 	($BANUSER['isChat'] == 1) ? true : false;
		...
		if ($BANUSER['banaday'] > TIMESTAMP)
		{
			...
			$SQL     .= "`email` = '". $mail ."', ";
			$SQL     .= "`isChat` = '". $chatban ."' ";
			$SQL     .= "WHERE `who` = '".$Name."' AND `universe` = '".Universe::getEmulated()."';";
		...
		$SQL     = "UPDATE ".USERS." SET ";
		$SQL    .= "`banaday` = '". $BannedUntil ."', ";
		$SQL	.= isset($_POST['chatban']) ? "`isChat` = '1'" : "`isChat` = '1'";
		$SQL    .= "WHERE ";
		$SQL    .= "`username` = '". $Name ."' AND `universe` = '".Universe::getEmulated()."';";
		$GLOBALS['DATABASE']->query($SQL);
		}

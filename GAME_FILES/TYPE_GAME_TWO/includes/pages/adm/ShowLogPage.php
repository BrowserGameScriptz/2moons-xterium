...
function ShowLogCampaignList() {
	global $LNG;
	$result    = $GLOBALS['DATABASE']->query("SELECT l.id, l.admin, l.time, l.universe, l.target,u_a.username as admin_username FROM ".LOG." as l LEFT JOIN ".USERS." as u_a ON  u_a.id = l.admin WHERE mode = 4 ORDER BY id DESC");
	...
	$targetkey = array(
		0 => 'Created a campaign',
		1 => 'Created a campaign [failed]',
	...
	$template->assign_vars(array(	
		...
		'pageTitle'		=> "Campaign Logs",
		'pageactiveshow1'=> "campaign",
	));
...
function ShowLogSettingsList() {
	...
	$targetkey = array(
		...
		4 => $LNG['log_tssettings'],
		5 => $LNG['log_disclamersettings'],
		6 => 'Configuration of the buildings queues',
		7 => 'Configuration of the referals bonus',
		8 => 'Configured of colonisation options',
		9 => 'Configured the start planet settings',
		10 => 'Configured the debris settings',
		11 => 'Configured the galaxy settings',
		12 => 'Configured the noob protections',
		13 => 'Configured various options',
		14 => 'Configured the meta options',
		15 => 'Configured server options'
	);
	...
	$template->assign_vars(array(	
		...
		'pageactiveshow'=> "Settings_Logs",
		'pageactiveshow1'=> "settings",
	));
	$template->show("LogList.tpl");
}
...
function ShowLogPlanetsList() {
	...
	$template->assign_vars(array(	
		...
		'log_id'		=> $LNG['log_id'],
		'log_view'		=> $LNG['log_view'],
		'pageTitle'		=> "Planets Logs",
		'pageactiveshow'=> "Planets_Logs",
		'pageactiveshow1'=> "planet",
	));
	$template->show("LogList.tpl");
}
...
function ShowLogPlayersList() {
	...
	$template->assign_vars(array(	
		...
		'log_id'		=> $LNG['log_id'],
		'log_view'		=> $LNG['log_view'],
		'pageTitle'		=> "Players Logs",
		'pageactiveshow'=> "Players_Logs",
		'pageactiveshow1'=> "player",
	));
	$template->show("LogList.tpl");
}
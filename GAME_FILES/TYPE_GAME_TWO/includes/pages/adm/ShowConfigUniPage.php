...
function ShowConfigUniPage()
{
	...
	if (!empty($_POST))
	{
		$config_before = array(
			...
			'support_auto'			=> $config->support_auto,
			...
			'special_donation_academy'	 	=> $config->special_donation_academy,
			...
			'timeFrom'	 	=> $config->timeRewardFrom,
			'timeTo'	 	=> $config->timeRewardTo,
			...
			'auctioneer_closure'	 	=> $config->auctioneer_closure,
			'alloEvent'	=> $config->alloEvent,
		...
		$special_donation_status  	= isset($_POST['special_donation_status']) && $_POST['special_donation_status'] == 'on' ? 1 : 0;
		$new_year_status  	= isset($_POST['new_year_status']) && $_POST['new_year_status'] == 'on' ? 1 : 0;
		$cosmonaute_status  	= isset($_POST['cosmonaute_status']) && $_POST['cosmonaute_status'] == 'on' ? 1 : 0;
		$auctioneer_closure  	= isset($_POST['auctioneer_closure']) && $_POST['auctioneer_closure'] == 'on' ? 1 : 0;
		$alloEvent  	= isset($_POST['alloEvent']) && $_POST['alloEvent'] == 'on' ? 1 : 0;
		$auctionExpe  	= isset($_POST['auctionExpe']) && $_POST['auctionExpe'] == 'on' ? 1 : 0;
		$primebuild  	= isset($_POST['primebuild']) && $_POST['primebuild'] == 'on' ? 1 : 0;
		$reg_closed 			= isset($_POST['reg_closed']) && $_POST['reg_closed'] == 'on' ? 1 : 0;
		$support_auto 			= isset($_POST['support_auto']) && $_POST['support_auto'] == 'on' ? 1 : 0;
		...
		$special_donation_up	 = HTTP::_GP('special_donation_up', 0);
		$ap_don	 = HTTP::_GP('ap_don', 0);
		...
		$timeFrom	 = HTTP::_GP('timeFrom', 0);
		$timeTo	 = HTTP::_GP('timeTo', 0);
		$timeReward	 = HTTP::_GP('timeReward', 0);
		...
		$config_after = array(
			...
			'auctioneer_closure' 	=> $auctioneer_closure,
			'alloEvent' 	=> $alloEvent,
			...
			'special_donation_up'	 	=> $special_donation_up,
			'ap_don'	 	=> $ap_don,
			'special_donation_academy'	 	=> $special_donation_academy,

...
abstract class AbstractGamePage
{
	...
	protected function Achievements()
	{
		...
		if ($onlineDatas['light_conveyor'] >= (1 * $USER['achievement_build_4']) + 1){
			...
			$metal_reward_am = min(20000,round($metal_reward_am * pow(1.15, $USER['achievement_build_4'])));
			$metal_reward_points = min(20000,round($metal_reward_points * pow(1.342, $USER['achievement_build_4'])));
		...
		if ($onlineDatas['medium_conveyor'] >= (1 * $USER['achievement_build_5']) + 1){
			...
			$metal_reward_am = min(20000,round($metal_reward_am * pow(1.16, $USER['achievement_build_5'])));
			$metal_reward_points = min(20000,round($metal_reward_points * pow(1.371, $USER['achievement_build_5'])));
		...
		if ($onlineDatas['heavy_conveyor'] >= (1 * $USER['achievement_build_6']) + 1){
			...
			$metal_reward_am = min(20000,round($metal_reward_am * pow(1.17, $USER['achievement_build_6'])));
			$metal_reward_points = min(20000,round($metal_reward_points * pow(1.41, $USER['achievement_build_6'])));
		...
		if ($USER['brotherhood'] >= (2 * $USER['achievement_tech_8']) + 2){
			...
			$metal_reward_am = min(20000,round($metal_reward_am * pow(1.30, $USER['achievement_tech_8'])));
			$metal_reward_points = min(20000,round($metal_reward_points * pow(1.30, $USER['achievement_tech_8'])));
	...
	protected function getPageData()
	{
		...
		if($USER['vote_sys_1'] < TIMESTAMP - 12*3600 || $USER['vote_sys_2'] < TIMESTAMP - 12*3600 || $USER['vote_sys_4'] < TIMESTAMP - 12*3600 ){
			$showVoteMenu = 1;	
		}

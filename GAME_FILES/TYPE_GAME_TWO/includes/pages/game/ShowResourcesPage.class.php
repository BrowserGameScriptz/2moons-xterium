...
class ShowResourcesPage extends AbstractGamePage
{
	...
	function show()
	{
			...
			foreach($ressIDs as $ID) 
			{
				...
				if($USER['ally_id'] != 0){
					...
					if($ALLIANCE['ally_fraction_id'] != 0 && $ALLIANCE['ally_fraction_level'] != 0){
						...
						$ally_fraction_resorc_prod = $FRACTIONS['ally_fraction_resource_prod'] * $ALLIANCE['ally_fraction_level'];
					}
				}
		...
		$METALFRACT   = ((30*25 * $BuildLevel * pow((1.1), $BuildLevel)) * (0.1 * $BuildLevelFactor) / 100 * $ally_fraction_resorc_prod) * $config->resource_multiplier; 
		...
		$CRYSACADAEMY = ((20*25 * $BuildLevelC * pow((1.1), $BuildLevelC)) * (0.1 * $BuildLevelCFactor) / 100 * $academy_p_b_2_1201) * $config->resource_multiplier; 
		...
		$CRYSTFRACT   = ((20*25 * $BuildLevelC * pow((1.1), $BuildLevelC)) * (0.1 * $BuildLevelCFactor) / 100 * $ally_fraction_resorc_prod) * $config->resource_multiplier; 
		...
		$DEUTEFRACT   = ((10*25 * $BuildLevelD * pow((1.1), $BuildLevelD) * (-0.002 * $BuildTemp + 1.28) * (0.1 * $BuildLevelDFactor)) / 100 * $ally_fraction_resorc_prod) * $config->resource_multiplier;
		...
		$ENERDALLIANC = 0; 
		$ENERDFRACTIO = $ENERDDEFAULT / 100 * $ally_fraction_energy_prod; 
			
		$this->assign(array(
			'header'			=> sprintf($LNG['rs_production_on_planet'], $PLANET['name']),
			'ENERDFRACTIO'		=> $ENERDFRACTIO,
			'DEUTEFRACT'		=> $DEUTEFRACT,
			'CRYSTFRACT'		=> $CRYSTFRACT,
			'METALFRACT'		=> $METALFRACT,

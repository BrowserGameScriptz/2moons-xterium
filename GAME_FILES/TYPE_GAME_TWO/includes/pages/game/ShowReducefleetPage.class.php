		...
		if($PLANET['gal6mod'] == 1){
			...
			foreach($_POST['palanets'] as $ID => $Value) {
				...
				if($FleetRoom < $consumption){
					$newTestFleetRool = max(1,$speed-1);
					for ($i = 1; $i <= $newTestFleetRool; $i++) {
						$duration      				= max(5,FleetFunctions::GetMissionDuration($newTestFleetRool, $MaxFleetSpeed, $distance, $GameSpeedFactor, $USER));	
						$consumption   				= FleetFunctions::GetFleetConsumption($rawfleetarray, $duration, $distance, $USER, $GameSpeedFactor);
						
						if($FleetRoom < $consumption) continue;
						
						if($FleetRoom >= $consumption) break;
					}
					
					if($FleetRoom < $consumption){
						$logger = new Logger();
						$logger->log(date('m-d-Y H:i:s') . ' ' . $_SERVER['REMOTE_ADDR']);
						$logger->log('RESULT: The capacity is inferior then the deut comsumption');
						$FleetErrorConso++;
						continue; 
					}
				}

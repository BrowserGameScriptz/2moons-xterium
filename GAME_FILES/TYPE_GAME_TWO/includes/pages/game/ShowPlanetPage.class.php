...
class ShowPlanetPage extends AbstractGamePage
{
	...
	function dimeter() 
	{
		...
		$der_metal_mod = $PLANET['der_metal'];
		$der_crysl_mod = $PLANET['der_crystal'];
		if($PLANET['planet_type'] == 3)
		{
			// Use planet debris, if attack on moons
			$sql			= "SELECT der_metal, der_crystal FROM %%PLANETS%% WHERE id_luna = :moonId;";
			$targetDebris	= database::get()->selectSingle($sql, array(
				':moonId'	=> $PLANET['id']
			));
			$der_metal_mod = $targetDebris['der_metal'];
			$der_crysl_mod = $targetDebris['der_crystal'];
		}
		
		if($der_metal_mod >= 100000000000 && $der_crysl_mod >= 50000000000 && $USER['stardust'] >= 1 && $PLANET['planet_type'] == 1){
			$posDiamter = mt_rand(276,414);
			$posFields  = mt_rand(12,18);
			$PLANET['der_metal'] -= 100000000000;
			$PLANET['der_crystal'] -= 50000000000;
			$USER['stardust'] -= 1;
			$sql	= "UPDATE %%USERS%% SET stardust = stardust - 1 WHERE id = :userId;";
			database::get()->update($sql, array(
				':userId'		=> $USER['id']
			));
			$sql	= "UPDATE %%PLANETS%% SET diameter = diameter + :diameter, field_max = field_max + :field_max, der_metal = der_metal - 100000000000, der_crystal = der_crystal - 50000000000 WHERE id = :planetId;";
			database::get()->update($sql, array(
				':diameter'		=> $posDiamter,
				':field_max'	=> $posFields,
				':planetId'		=> $PLANET['id']
		...
		}elseif($der_metal_mod >= 50000000000 && $der_crysl_mod >= 25000000000 && $USER['stardust'] >= 1 && $PLANET['planet_type'] == 3){
			...
			$targetDebris['der_metal'] -= 50000000000;
			$targetDebris['der_crystal'] -= 25000000000;
			$USER['stardust'] -= 1;
			$sql	= "UPDATE %%USERS%% SET stardust = stardust - 1 WHERE id = :userId;";
			database::get()->update($sql, array(
				':userId'		=> $USER['id']
			...
			$sql	= "UPDATE %%PLANETS%% SET der_metal = der_metal - 50000000000, der_crystal = der_crystal - 25000000000 WHERE id_luna = :moonId;";
			database::get()->update($sql, array(
				':moonId'		=> $PLANET['id']
			));
	...
	function show()
	{
		...
		$der_crystal 	= $PLANET['der_crystal'];
		...
		if($planetMod['planet_type'] == 3){
			...
			$der_crystal = $planetModBis['der_crystal'];
		}
		...
		$this->assign(array(
			...
			'pder_metalx' 			=> $der_metal,
			'pder_crystalx' 		=> $der_crystal,

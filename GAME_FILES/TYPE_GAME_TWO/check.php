...
function getClientIp()
{
	...
	if (!empty($loginData)){
		...
		$sql = "SELECT COUNT(*) as count FROM %%TIMEBONUS%% WHERE userID = :userid";
		$TIMEBONUS = database::get()->selectSingle($sql, array(
			':userid'	=> $loginData['id']
		));
			
		if($config->timeRewardFrom < TIMESTAMP && $config->timeRewardTo > TIMESTAMP && empty($TIMEBONUS['count'])){
			$sql = "UPDATE %%USERS%% SET academy_p = academy_p + 250 WHERE id = :loginID";
			database::get()->update($sql, array(
				':loginID'		=> $loginData['id']
			));
			$sql = "INSERT INTO %%TIMEBONUS%% SET userID = :userID, TIMESTAMP = :TIMESTAMP";
